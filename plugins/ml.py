...
def execute(params):
    ...
    try:
        ...
        if task == "sentiment":
            ...
            return {
                "success": True,
                "result": f"Sentiment: {label} ({score}% confidence)",
            }
            ...
            if not question or not context:
                return {
                    "success": False,
                    "result": "Need 'question' and 'context' params",
                }
            ...
            return {
                "success": True,
                "result": f"Answer: {result['answer']} (score: {round(result['score'], 3)})",
            }
            ...
            return {
                "success": False,
                "result": f"Unknown task: {task}. Use: sentiment, qa, zero_shot",
            }

    except ImportError:
        return {
            "success": False,
            "result": "transformers not installed: pip install transformers torch",
        }
