...
PLUGIN_PARAMS = {
    "action": "list|copy|move|delete|mkdir|search|read|write|info",
    "path": "target path",
    "dest": "destination (for copy/move)",
    ...
    try:
        ...
        if action == "list":
            ...
            for f in files[:30]:
                ...
                try:
                    ...
                    mtime = datetime.fromtimestamp(os.path.getmtime(fp)).strftime(
                        "%Y-%m-%d %H:%M"
                    )
                except Exception:
                    sz, mtime = "?", "?"
        ...
        # SEARCH
            ...
            if not matches:
                return {
                    "success": True,
                    "result": f"No files matching '{pattern}' in {base}",
                }
            return {
                "success": True,
        ...
        # READ
            ...
            return {
                "success": True,
                "result": f"--- {path} ({lines_count} lines) ---\n{text}",
            }
        ...
        # WRITE
            ...
            if not content:
                return {
                    "success": False,
                    "result": "WRONG TOOL: Use write_file tool to write files, not file_manager(action=write). Example: write_file(path='...', content='...')",
                }
            ...
            return {
                "success": True,
                "result": f"Written {len(content)} chars to {path}. NOTE: Use write_file tool instead of file_manager for writing.",
            }

        else:
            return {
                "success": False,
                "result": f"Unknown action '{action}'. Use: list, info, copy, move, delete, mkdir, search, read, write",
            }
