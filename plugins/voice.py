...
def _load_tts_settings() -> dict:
    try:
        ...
        return {"provider": "piper", "api_key": "", "model": "", "language": "de-DE", "voice": ""}
...
def _speak_google(text: str, settings: dict):
    ...
    if voice_name:
        ...
        voice_config["ssmlGender"] = "MALE" if any(x in voice_name.upper() for x in ("-B", "-D")) else "FEMALE"
    else:
...
def _speak_elevenlabs(text: str, settings: dict):
    ...
    voice_id = settings.get("voice", "") or settings.get("model", "") or "pNInz6obpgDQGcFmaJgB"
    ...
    while True:
        try:
            ...
            if speak_fn:
                ...
                log.error(f"Unbekannter TTS Provider: '{provider}'. Verfügbar: {list(PROVIDERS.keys())}")
