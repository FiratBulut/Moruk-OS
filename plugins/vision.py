...
PLUGIN_DESCRIPTION = "Computer Vision Tools: snapshot, face_detect, qr_detect. Saves ~/moruk-os/vision_*.jpg. Modes: snapshot|face|qr"
PLUGIN_PARAMS = ["mode"]
...
def execute(params):
    if not _cv2_available:
        return {"success": False, "result": "opencv-python not installed. Run: pip install opencv-python --break-system-packages"}
    ...
    if mode == "face":
        ...
        cv2.putText(frame, f"Faces: {len(faces)}", (15, 25), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 0), 2)
        ...
        cv2.putText(frame, f"QR: {text}", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 1)
        detail = f"QR: {text}"
