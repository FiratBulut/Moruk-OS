...
PLUGIN_CORE = False
PLUGIN_DESCRIPTION = "Generiert Bilder basierend auf einem Text-Prompt mithilfe der Gemini/Imagen API."
PLUGIN_PARAMS = {
...
def execute(params):
    ...
    if not api_key:
        return {"success": False, "result": "Fehler: Kein API-Key für Vision/Imagen in Settings gefunden. Bitte Vision Slot konfigurieren."}
    ...
    is_gemini_image = any(x in model.lower() for x in ["gemini-3-pro-image", "gemini-2.5-flash-image",
                                                         "gemini-3.1-flash-image", "flash-image", "pro-image"])
    ...
    try:
        ...
        if response.status_code != 200:
            return {"success": False, "result": f"API Fehler {response.status_code}: {response.text[:300]}"}
        data = response.json()
        ...
        if "predictions" in data:
            for pred in data.get("predictions", []):
                image_data = pred.get("bytesBase64Encoded") or pred.get("image", {}).get("imageBytes")
                if image_data:
                    break
        ...
        if not image_data:
            ...
            return {"success": False, "result": f"Keine Bilddaten erhalten (Model: {model})\nAPI Response: {snippet}"}
        ...
        return {"success": True, "result": f"Bild erfolgreich generiert und gespeichert: {filepath}\n\n![Generated Image]({filepath})"}
