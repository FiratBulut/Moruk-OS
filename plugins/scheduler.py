...
PLUGIN_PARAMS = {
    "action": "schedule|list|cancel|run",
    "task": "Task-Name",
    "time": "Zeit (cron oder +Xm, +Xh)",
    "command": "Befehl oder Plugin-Aufruf",
}
...
SCHEDULE_FILE = os.path.join(BASE_DIR, "data", "scheduled_tasks.json")
...
BLOCKED_COMMANDS = [
    ...
    "halt",
    "write_file",
    "self_edit",  # Keine Datei-Schreiboperationen
]
...
def _is_safe_command(command: str) -> tuple[bool, str]:
    ...
    SAFE_PREFIXES = (
        ...
        "mkdir ",
        "cp ",
        "mv ",  # nur kopieren/verschieben erlaubt
        "git ",
        "curl ",
        "wget ",
    )
    if not any(cmd_lower.startswith(p) for p in SAFE_PREFIXES):
        return (
            False,
            f"Befehl nicht in Whitelist. Erlaubt: python3, pip, ls, cat, grep, find, echo, mkdir, cp, mv, git, curl, wget",
        )
    return True, ""
...
def execute(params):
    ...
    # ── SCHEDULE ━━━━━━━━━━━━━━━━━━━━━━━━━━
        ...
        if not task_name or not time_spec or not command:
            return {
                "success": False,
                "result": "Provide: task, time (+Xm/+Xh/+Xd), command",
            }
        ...
        if time_spec.startswith("+"):
            try:
                ...
                return {
                    "success": False,
                    "result": "Invalid time format. Use +30m, +2h, +1d",
                }
        else:
        ...
        if len(pending) >= MAX_SCHEDULED_TASKS:
            return {
                "success": False,
                "result": f"❌ Scheduler voll: Max {MAX_SCHEDULED_TASKS} Tasks erlaubt. Erst bestehende abbrechen.",
            }
        ...
        return {
            "success": True,
            "result": f"Scheduled '{task_name}' at {run_at_fmt}\nCommand: {command}",
        }
    ...
    # ── RUN (immediate) ━━━━━━━━━━━━━━━━━━━━━━━
        ...
        try:
            ...
            return {
                "success": False,
                "result": f"Command timed out after 60s: {command}",
            }
        except Exception as e:
        ...
        return {
            "success": False,
            "result": f"Unknown action '{action}'. Use: schedule, list, cancel, run",
        }
