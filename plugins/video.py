...
def _generate_fal(prompt: str, model: str, api_key: str, duration: int) -> dict:
    ...
    if not api_key:
        return {
            "success": False,
            "result": "fal.ai: kein API Key (video_api_key in Settings)",
        }
    if not model:
    ...
    payload = json.dumps(
        {"prompt": prompt, "duration": duration, "aspect_ratio": "16:9"}
    ).encode()
    ...
    try:
        ...
        return {
            "success": True,
            "result": f"Video gespeichert: {out_path}",
            "path": str(out_path),
            "url": video_url,
        }
...
def _generate_google_veo(prompt: str, model: str, api_key: str, duration: int) -> dict:
    ...
    if not key:
        return {
            "success": False,
            "result": "Google Veo: kein API Key (video_api_key in Settings eintragen)",
        }
    ...
    try:
        ...
        videos = (
            op.get("response", {}).get("videos")
            or op.get("response", {}).get("generatedSamples")
            or []
        )
        if not videos:
            return {"success": False, "result": f"Veo: kein Video in Response — {op}"}

        video_url = videos[0].get("uri") or videos[0].get("video", {}).get("uri", "")
        if not video_url:
        ...
        return {
            "success": True,
            "result": f"Veo Video gespeichert: {out_path}",
            "path": str(out_path),
        }
...
def _generate_openai(prompt: str, model: str, api_key: str, duration: int) -> dict:
    ...
    try:
        ...
        response = client.videos.generate(
            model=model or "sora", prompt=prompt, duration=duration
        )
...
def _analyze_frames(path: str, num_frames: int, mode: str) -> dict:
    if not _cv2_available:
        return {
            "success": False,
            "result": "opencv-python fehlt: pip install opencv-python --break-system-packages",
        }
    ...
    if mode == "summarize":
        return {
            "success": True,
            "result": f"{len(frames)} Frames aus {os.path.basename(path)} extrahiert.",
            "frames": frames,
        }
    return {
        "success": True,
        "result": f"{len(frames)} Frames: {frames}",
        "frames": frames,
    }
...
def execute(params):
    ...
    if mode in ("frames", "summarize", "analyze", "motion"):
        return _analyze_frames(
            params.get("path", ""), int(params.get("num_frames", 5)), mode
        )
    ...
    if not gen_fn:
        return {
            "success": False,
            "result": f"Unbekannter Provider: '{provider}'. Verfuegbar: {list(GENERATION_PROVIDERS.keys())}",
        }
