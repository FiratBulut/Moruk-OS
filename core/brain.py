"""
Moruk AI OS - Brain Module v2.1
Provider-agnostischer LLM Abstraction Layer mit Tool-Loop.

Fixes v2.1:
- _audit_tool_call: DeepThink blockiert nicht mehr, nur Advisory + lokaler Shield
- _filter_messages: Trailing-pop Bug gefixt (Assistant-Nachrichten bleiben erhalten)
- validate_action: Entfernt (war rekursiv und blockierend)
- Thread-Safety: conversation_history wird mit Lock geschützt
"""
...
class Brain:
    ...
    def __init__(self):
        ...
        self._history_lock = threading.Lock()  # NEU: Thread-Safety
        self.client = None
        ...
        self.log.info(
            f"Brain initialized. Provider: {self.settings.get('provider')}, configured: {self.is_configured()}"
        )
    ...
    def _init_client(self):
        ...
        try:
            if provider == "anthropic":
                ...
                base_url = (
                    model_base_url
                    or self.settings.get("providers", {})
                    .get("ollama", {})
                    .get("base_url", "")
                ...
                from openai import OpenAI
                self.client = (
                    OpenAI(base_url=model_base_url, api_key=api_key)
                    if model_base_url
                ...
                if base_url:
                    ...
                    self.client = None
                    self.log.warning(
                        f"Provider '{provider}': kein base_url. In Settings eintragen."
                    )

            if self.client:
                effective_url = model_base_url or self.PROVIDER_BASE_URLS.get(
                    provider, "default"
                )
                self.log.info(
                    f"Client ready: provider={provider} ({self._provider_type}), "
    ...
    def _init_deepthink(self):
        try:
            ...
            if self.deepthink.is_enabled():
                self.log.info(
                    f"DeepThink enabled: {self.settings.get('deepthink_model')}"
                )
        except Exception as e:
            self.log.error(f"DeepThink init failed: {e}")
            self.deepthink = None

    # ── Security & Auditing Layer (v2.1 — Vereinfacht) ────────

    def _audit_tool_call(self, tool_name: str, params: dict, user_message: str) -> dict:
        """Sicherheitsprüfung vor Tool-Ausführung.
        
        v2.1 Änderungen:
        - Stufe 1: Lokaler Shield (harter Block nur für kritische Befehle)
        - Stufe 2: Syntax-Check für Python-Dateien
        - Stufe 3: DeepThink Advisory (nur Warnung, kein Block)
        - Stufe 4: Rollback-Backup vor Datei-Schreiboperationen
        
        DeepThink blockiert NICHT MEHR — nur noch lokaler Shield bei rm -rf etc.
        """
        RISKY_TOOLS = {"terminal", "write_file", "self_edit", "system_repair"}

        # ── Stufe 1: Lokaler Shield (harter Block nur für Katastrophen) ──
        if tool_name == "terminal":
            cmd = params.get("command", "").strip().lower()
            CRITICAL = ["rm -rf /", "mkfs", "dd if=", "> /dev/sda", "chmod -R 777 /"]
            if any(f in cmd for f in CRITICAL):
                self.log.warning(f"🛡 Shield blockiert: {cmd[:80]}")
                return {
                    "approved": False,
                    "reason": f"Shield: Kritisch destruktiver Befehl ({cmd[:60]})",
                }
        ...
        if tool_name in ("write_file", "self_edit"):
            ...
            if path.endswith(".py") and content_to_write:
                ...
                try:
                    ...
                    return {
                        "approved": False,
                        "reason": f"Syntax-Fehler verhindert Schreiben: {se}. Bitte korrigiere den Code.",
                    }

        # ── Stufe 3: DeepThink Advisory (nur Log, KEIN Block) ──
        if tool_name in RISKY_TOOLS and self.deepthink and self.deepthink.is_enabled():
            advisory = self.deepthink.advisory_check(tool_name, params)
            if advisory.get("severity") == "high":
                # NUR bei severity=high wird blockiert (rm -rf etc.)
                # Das ist redundant mit Stufe 1, aber doppelt hält besser
                self.log.warning(f"🛡 DeepThink Advisory HIGH: {advisory['warning']}")
                return {"approved": False, "reason": f"Advisory: {advisory['warning']}"}
            elif advisory.get("warning"):
                # Low severity → nur loggen, NICHT blockieren
                self.log.info(f"⚠ DeepThink Advisory: {advisory['warning']}")
...
"""
    ...
    def think(
        ...
        # DeepThink Force Mode & Direct Mode
        _force = force_deepthink and self.deepthink and self.deepthink.is_enabled()
        _direct = (
            self.deepthink and self.deepthink.should_think_deep(user_message)
            if self.deepthink
            else False
        )

        if _force or _direct:
            with self._history_lock:
        ...
        self.log.info(
            f"Think: depth={depth}, system_prompt_len={len(system.split())} words, max_iter={max_iterations}"
        )

        # Thread-safe: isolated bekommt eigene Liste, shared bekommt Kopie
        if isolated:
            active_history = []
        else:
            with self._history_lock:
        ...
        try:
            ...
            iteration = 0
            self.log.info(
                f"Think start: '{user_message[:80]}' (isolated={isolated}, iterations max: {max_iterations})"
            )
            ...
            while iteration < max_iterations:
                ...
                if use_stream:
                    response_text = self._call_provider_stream(
                        system, on_token=on_token, history=trimmed_history
                    )
                else:
                ...
                if self.tool_router and self.tool_router.has_tool_calls(response_text):
                    ...
                    for call in calls:
                        ...
                        # ── SICHERHEITS-LAYER ──
                        audit_result = self._audit_tool_call(
                            tool_name, params, user_message
                        )
                    ...
                    if response_text:
                        ...
                        self.log.warning(
                            f"Empty response from model on iteration {iteration}, retrying once..."
                        )
                    ...
                    # DeepThink Review (nur bei hohem Depth)
                    _dt_should = depth >= 5 or force_deepthink
                    if (
                        self.deepthink
                        and self.deepthink.is_enabled()
                        and _dt_should
                        and self.deepthink.should_review(
                            user_message, full_response, depth
                        )
                    ):
                        ...
                        if verdict == "revise" and review.get("suggestion"):
                            self.log.info(
                                f"DeepThink: revise — {review['suggestion'][:100]}"
                            )
                            active_history.append(
    ...
    def _filter_messages(self, messages: list) -> list:
        ...
        v2.1 Fix: Trailing assistant messages werden NICHT mehr entfernt.
        Stattdessen: Nur sicherstellen dass keine doppelten Rollen aufeinanderfolgen
        und dass die Konversation mit einer User-Message beginnt.
        """
        filtered = [
            m
            for m in messages
        ...
        # Merge aufeinanderfolgende gleiche Rollen
        merged = []
        for msg in rest:
        ...
        # Sicherstellen: erste Nicht-System-Message muss "user" sein
        while merged and merged[0]["role"] != "user":
            merged.pop(0)

        # v2.1 FIX: NICHT mehr am Ende poppen!
        # Das alte `while merged[-1]["role"] != "user": merged.pop()`
        # hat Assistant-Antworten im Tool-Loop gelöscht.

        final_msgs = (
            system_msgs + merged
            if merged
            else system_msgs + [{"role": "user", "content": "Continue."}]
        )
        return final_msgs
    ...
    def _call_anthropic(
        self, system: str, messages: list, model: str, max_tokens: int, temperature: float,
    ) -> str:
        filtered = [
            m for m in self._filter_messages(messages) if m.get("role") != "system"
        ]
    ...
    def _call_provider_stream(
        self, system_prompt: str, on_token=None, history: list = None
    ) -> str:
        model = self.settings.get("model", "")
        max_tokens = self.settings.get("max_tokens", 4096)
        temperature = self.settings.get("temperature", 0.7)
    ...
    def _call_anthropic_stream(
        self, system, messages, model, max_tokens, temperature, on_token=None
    ) -> str:
        full_text = ""
        filtered = [
            m for m in self._filter_messages(messages) if m.get("role") != "system"
                ...
                try:
                    ...
                    if final_msg and final_msg.usage:
                        self.last_token_usage = {
                            ...
                            "total": final_msg.usage.input_tokens
    ...
    def _call_openai_compatible(
        ...
        for attempt in range(3):
            try:
                ...
                if any(
                    x in error_str
                    for x in ("rate limit", "429", "too many requests", "ratelimit")
                ):
                    wait = 2**attempt * 5
                    self.log.warning(
                        f"Rate limit hit (attempt {attempt+1}/3) — waiting {wait}s..."
                    )
                    _time.sleep(wait)
                    continue
                ...
                if any(
                    x in error_str
                    for x in ("500", "context", "too long", "limit", "length")
                ):
                    for keep in (6, 4, 2):
                        if len(oai_messages) <= keep + 1:
                            continue
                        self.log.warning(
                            f"Retrying with last {keep} messages (context reduction)..."
                        )
                        reduced = [oai_messages[0]] + self._filter_messages(
                            oai_messages[-keep:]
                        )
                        try:
                            self._check_api_budget()
    ...
    def _check_api_budget(self):
        ...
        if total >= budget_limit * 0.8:
            self.log.warning(
                f"⚠ API Budget bei {total/budget_limit:.0%}: {total:,}/{budget_limit:,} Tokens"
            )
    ...
    def _get_vision_format(self) -> str:
        ...
        if not base_url:
            ...
            base_url = (
                self.settings.get("providers", {}).get(provider, {}).get("base_url", "")
            )
        ...
        if (
            ...
            or "googleapis.com" in base_url_lower
        ):
            return "gemini"
            ...
            if os.path.exists(img_path):
                try:
                    ...
                    content_parts.append(
                        {"type": "text", "text": f"[Image load error: {e}]"}
                    )
            else:
                content_parts.append(
                    {"type": "text", "text": f"[Image not found: {img_path}]"}
                )
