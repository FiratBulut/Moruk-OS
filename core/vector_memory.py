"""
Moruk AI OS - Vector Memory v2.1
SQLite + TF-IDF basiertes semantisches Gedächtnis.

v2.1 Fixes:
- store() hat neuen skip_dedup Parameter für Bulk-Inserts (index_codebase)
- _rebuild_tfidf() wird bei Bulk-Insert nur einmal am Ende aufgerufen
- search() mit robusterem Error-Handling
"""
...
class VectorMemory:
    ...
    def store(self, content, category="learned", tags=None, skip_dedup=False):
        """Speichert einen Eintrag.
        
        Args:
            skip_dedup: Wenn True, wird der teure Duplikat-Check übersprungen.
                        Nützlich für Bulk-Operationen wie index_codebase().
        """
    ...
    def search(
        self, query: str, max_results: int = 5, min_score: float = 0.1, **kwargs
    ):
        limit = kwargs.get("limit", max_results)
        category_filter = kwargs.get("category", None)
        ...
        with self._lock:
            ...
            try:
                ...
                results = []
                for idx, score in valid_matches:
        ...
        with self._lock:
            try:
                ...
                for (raw_tags,) in self.conn.execute(
                    "SELECT tags FROM memories WHERE tags IS NOT NULL"
                ):
                    try:
                ...
                db_size = (
                    os.path.getsize(self.db_path) / 1024
    ...
    def close(self):
        with self._lock:
            try:
                self.conn.close()
                log.info("Vector memory database closed cleanly.")
            except Exception as e:
                log.error(f"Error closing vector memory db: {e}")
