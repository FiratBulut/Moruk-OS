...
def sync_and_call(obj, method_name, *args, **kwargs):
    ...
    try:
        ...
        logging.warning(
            f"[Safe-Sync] Methode {method_name} nicht gefunden in {module_name}. Erzwinger Reload..."
        )
        ...
        if module_name in sys.modules:
            ...
            if hasattr(new_class, method_name):
                ...
                logging.info(
                    f"[Safe-Sync] Synchronisation erfolgreich: {method_name} wurde nachgeladen."
                )
                return getattr(obj, method_name)(*args, **kwargs)

        raise AttributeError(
            f"Methode {method_name} existiert auch nach Reload nicht auf Disk."
        )
